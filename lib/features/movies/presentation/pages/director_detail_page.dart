...
class _DirectorDetailPageState extends State<DirectorDetailPage> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final crew = (person['movie_credits'] as Map<String, dynamic>?)?['crew']
              as List? ??
          [];
      final directed = crew.where((c) => c['job'] == 'Director').map((j) {
        return Movie(
      ...
      directed.sort((a, b) => b.releaseDate.compareTo(a.releaseDate));
      if (!mounted) return;
      setState(() {
        _person = person;
        _filmography = directed;
        _isLoading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final name =
        _person?['name'] as String? ?? widget.data?['name'] as String? ?? '';
        ...
        child: ListView(
          ...
          children: [
            ...
            if (_filmography.isNotEmpty) ...[
              ...
              ActorSectionHeader(
                  title:
                      '${t.director.filmography} (${_filmography.length})'),
                      ...
                      child: SizedBox(
                        ...
                        child: Padding(
                          ...
                          child: Column(children: [
                            Expanded(
                                child:
                                    MoviePosterCard(height: 200, movie: movie)),
                            const SizedBox(height: 6),
                            Text(movie.title,
                                maxLines: 2,
